...
func maxProfitV3(prices []int) int {
	//dp[i][0] 表示第i天第1次持有股票的现金    dp[i][1] 表示第i天第1次不持有股票的现金  dp[i][2]表示第i天第2次持有股票的现金  dp[i][3]表示第i天第2次不持有股票的现金
	...
	return max(max(dp[len(prices)-1][0], dp[len(prices)-1][1]), max(dp[len(prices)-1][2], dp[len(prices)-1][3]))
}
